...
const BankTransferModal = ({ 
  ...
  onClose, 
  cartItems = [], 
  restaurantName = "The Place Restaurant",
  restaurantContact = "+234 8183944982",
  riderContact = "+234 8158411808"
}) => {
  ...
  const createOrderData = () => {
    const now = new Date();
    const orderId = `uty-${Math.random().toString(36).substr(2, 9)}`;
    
    const formattedItems = cartItems.map(item => ({
      name: item.name || item.title || item.productName || "Unknown Item",
    ...
    return {
      ...
      deliveryTime: now.toLocaleString('en-US', { 
        hour: 'numeric', 
        minute: 'numeric', 
        hour12: true,
        month: 'short',
    ...
    <>
      {showOrders ? (
        <OrdersModal 
          ...
          deliveredOrder={createOrderData()}
        />
      ) : !showConfirmation ? (
        ...
        <OrderConfirmationModal
          ...
          onViewOrder={() => setShowOrders(true)}
        />
      )}
    </>
  );
